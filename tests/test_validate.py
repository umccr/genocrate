...
def test_validate_batch_valid_bagit():
    """
    Test 'validate-batch' command succeeds for a batch conforming to the BagIt specification.
    """
    runner = CliRunner()
...
def test_validate_batch_invalid_bagit():
    """
    Test 'validate-batch' command fails for a batch NOT conforming to the BagIt specification.
    """
    runner = CliRunner()
    with patch('genocrate.commands.validate_batch.is_ro_crate_valid', return_value=True):
        result = runner.invoke(
            cli,
            ["validate-batch", "./tests/fixtures/batch-004", "-t", "bagit"]
        )
        assert result.exit_code == 1

def test_validate_batch_valid_md5():
    """
    Test 'validate-batch' command succeeds when the manifest is valid and all files are listed in the manifest.
    """
    runner = CliRunner()
...
def test_validate_batch_invalid_md5():
    """
    Test 'validate-batch' when the manifest failed (incorrect checksum or files not listed).
    """
    runner = CliRunner()
...
def test_validate_invalid_ro_crate():
    """
    Test 'validate-batch' command fails when the RO-Crate metadata does not list all files.
    """
    runner = CliRunner()
